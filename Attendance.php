...
<div class="panel-heading">
    <h1 style="text-align: center;padding: 50px;">Attandence</h1>
</div>

<div  class="plain body">
    <a href="adminview.php" class="btn btn-primary" style="margin: 10px;">View</a>
    <a href="sign up.php" class="btn btn-primary pull-right" style="float: right; margin:10px">Add</a> 
...
<form method="post">
 <table class="table">
  ...
  <tr>
        ...
        <td>
            present<input required type="radio" name="attendence[<?php echo $show['id'] ?>]" value="Present"
            >Absent<input required name="attendence[<?php echo $show['id'] ?>]" value="Absent" type="radio">
                  ...
                  if($result->num_rows>0){
                    while($check=$result->fetch_assoc()){
                      ...
                      if($date==$check['date']){
                        ...
                        `     Attendance alreday taken today!!;
                        </div>";
                      }
                    }
                  }
                    ...
                    if(!$b){
                      foreach($att as $key => $value){
                          ...
                          if($value=='Present'){
                             $query="insert into attendence(value,rollno,date) value('Present',$key,'$date')";
                             $insertResult=$conn->query($query);

                          }
                          if($value=='Absent'){
                            $query="insert into attendence(value,rollno,date) value('Absent',$key,'$date')";
                            $insertResult=$conn->query($query);
                          }
...
<input class="btn btn-primary" type="submit" value="Attendence">
</form> 
</div>
</div>
    <!-- Optional JavaScript -->
